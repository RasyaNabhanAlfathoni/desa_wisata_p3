        ...
        <div class="row justify-content-center">
            <div class="col-12">
                ...
                <div class="card shadow">
                    <div class="card-body">
                        <form action="{{ route('kelola_paket_wisata.update', $paket_wisata->id) }}" id="frmPaketWisata" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')

                            <div class="form-group">
                                <label for="nama_paket">Nama Paket Wisata</label>
                                <input type="text" class="form-control" name="nama_paket" id="nama_paket" required value="{{ old('nama_paket', $paket_wisata->nama_paket) }}">
                            </div>

                            <div class="form-group">
                                <label for="deskripsi">Deskripsi</label>
                                <textarea class="form-control" name="deskripsi" id="deskripsi" rows="4" required>{{ old('deskripsi', $paket_wisata->deskripsi) }}</textarea>
                            </div>

                            <div class="form-group">
                                <label for="fasilitas">Fasilitas</label>
                                <input type="text" class="form-control" name="fasilitas" id="fasilitas" required value="{{ old('fasilitas', $paket_wisata->fasilitas) }}">
                            </div>

                            <div class="form-group">
                                <label for="durasi_hari">Durasi (hari)</label>
                                <input type="number" class="form-control" name="durasi_hari" id="durasi_hari" required min="1" value="{{ old('durasi_hari', $paket_wisata->durasi_hari) }}">
                            </div>

                            <div class="form-group">
                                <label for="harga_per_pack">Harga Paket (Rp)</label>
                                <input type="number" class="form-control" name="harga_per_pack" id="harga_per_pack" required value="{{ old('harga_per_pack', $paket_wisata->harga_per_pack) }}">
                            </div>

                            <div class="form-group">
                                <label for="kuota_peserta">Kuota Peserta</label>
                                <input type="number" class="form-control" name="kuota_peserta" id="kuota_peserta" required value="{{ old('kuota_peserta', $paket_wisata->kuota_peserta) }}">
                            </div>

                            <div class="form-group">
                                <label for="nilai_diskon">Nilai Diskon (%)</label>
                                <input type="number" class="form-control" name="nilai_diskon" id="nilai_diskon" step="0.1" min="0" max="100" value="{{ old('nilai_diskon', $paket_wisata->nilai_diskon) }}">
                            </div>

                            <div class="form-group">
                                <label for="peserta_diskon">Jumlah Peserta Minimum untuk Diskon</label>
                                <input type="number" class="form-control" name="peserta_diskon" id="peserta_diskon" value="{{ old('peserta_diskon', $paket_wisata->peserta_diskon) }}">
                            ...
                            <div class="form-group">
                                ...
                                @for ($i = 1; $i <= 5; $i++)
                                    <div class="mb-3">
                                        ...
                                        @if(!empty($paket_wisata["foto$i"]))
                                            <small class="text-muted">Foto sebelumnya:</small>
                                            <img id="preview{{ $i }}" src="{{ asset('storage/' . $paket_wisata["foto$i"]) }}" alt="Preview Foto" style="width:150px; margin-top:10px;">
                                        @else
                                            <small class="text-muted">Foto sebelumnya:</small>
