        ...
        <div class="row justify-content-center">
            <div class="col-12">
                ...
                <div class="card shadow">
                    <div class="card-body">
                        <form action="{{ route('kelola_data_karyawan.update', $karyawan->id) }}" method="POST" id="frmKaryawan" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <div class="form-group">
                                <label for="nama_karyawan">Nama Karyawan</label>
                                <input type="text" class="form-control" name="nama_karyawan" id="nama_karyawan" value="{{ $karyawan->nama_karyawan }}" required>
                            </div>
                            <div class="form-group">
                                <label for="email">Email</label>
                                <input type="email" class="form-control" name="email" id="email" value="{{ $karyawan->user->email }}" required>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="aktif" id="aktif" required>
                                    <option value="1" {{ old('aktif', $karyawan->user->aktif) == 1 ? 'selected' : '' }}>Aktif</option>
                                    <option value="0" {{ old('aktif', $karyawan->user->aktif) == 0 ? 'selected' : '' }}>Nonaktif</option>
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="no_hp">No HP</label>
                                <input type="text" class="form-control" name="no_hp" id="no_hp" value="{{ $karyawan->no_hp }}" required>
                            </div>
                            <div class="form-group">
