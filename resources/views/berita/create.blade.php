        ...
        <div class="row justify-content-center">
            <div class="col-12">
                ...
                <div class="card shadow">
                    <div class="card-body">
                        <form action="{{ route('kelola_berita.store') }}" id="frmBerita" method="POST" enctype="multipart/form-data">
                            @csrf
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control" name="berita" id="berita" rows="8" required>{{ old('berita') }}</textarea>
                            </div>

                            <div class="form-group">
                                <label for="tgl_post">Tanggal Posting</label>
                                <input type="datetime-local" class="form-control" name="tgl_post" id="tgl_post"
                                       value="{{ old('tgl_post', now()->format('Y-m-d\TH:i')) }}" required>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="id_kategori_berita" id="id_kategori_berita" required>
                                    ...
                                    @foreach($kategori_beritas as $kategori)
                                        <option value="{{ $kategori->id }}" {{ old('id_kategori_berita') == $kategori->id ? 'selected' : '' }}>
                            ...
                            <!-- Upload Foto -->
                            <div class="form-group">
                                <label for="foto">Foto Beita</label>
                                <input type="file" class="form-control" name="foto" id="foto" accept="image/*" required>
                                <div id="foto" class="form-text text-warning">Unggah Foto Berita Anda!</div>
                            </div>
