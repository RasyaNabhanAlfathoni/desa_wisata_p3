        ...
        <div class="row justify-content-center">
            <div class="col-12">
                ...
                <div class="card shadow">
                    <div class="card-body">
                        <form action="{{ route('kelola_reservasi.update', $reservasi->id) }}" id="frmReservasi" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            ...
                            {{-- <div class="form-group">
                                ...
                                <input type="datetime-local" name="tgl_reservasi_wisata" id="tgl_reservasi_wisata" class="form-control"
                                       value="{{ old('tgl_reservasi_wisata', $reservasi->tgl_reservasi_wisata ? \Carbon\Carbon::parse($reservasi->tgl_reservasi_wisata)->format('Y-m-d\TH:i') : '') }}">
                                @error('tgl_reservasi_wisata') <div class="text-danger">{{ $message }}</div> @enderror
                            </div> --}}
                            ...
                            <div class="form-group">
                                ...
                                <input type="datetime-local" name="tgl_reservasi_akhir" id="tgl_reservasi_akhir" class="form-control" value="{{ old('tgl_reservasi_akhir', $reservasi->tgl_reservasi_akhir) }}" readonly required>
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" name="harga" id="harga" class="form-control" value="{{ old('harga', $reservasi->harga) }}" readonly>
                            ...
                            <div class="form-group">
                             ...
                             <div class="form-group">
                                ...
                                <input type="number" name="nilai_diskon" id="nilai_diskon" class="form-control" value="{{ old('nilai_diskon', $reservasi->nilai_diskon) }}" readonly>
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" name="diskon" id="diskon" class="form-control" value="{{ old('diskon', $reservasi->diskon) }}" readonly>
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" name="total_bayar" id="total_bayar" class="form-control" value="{{ old('total_bayar', $reservasi->total_bayar) }}" readonly>
                            ...
                            <div class="form-group">
                                ...
                                <div id="file_bukti_tf_help" class="form-text text-warning">Unggah File Bukti TF Reservasi! ( Maks 3MB )</div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="status_reservasi_wisata" id="status_reservasi_wisata" class="form-control">
                                    <option value="pesan" {{ old('status_reservasi_wisata', $reservasi->status_reservasi_wisata) == 'pesan' ? 'selected' : '' }}>Pesan</option>
                                    <option value="dibayar" {{ old('status_reservasi_wisata', $reservasi->status_reservasi_wisata) == 'dibayar' ? 'selected' : '' }}>Dibayar</option>
                                    <option value="selesai" {{ old('status_reservasi_wisata', $reservasi->status_reservasi_wisata) == 'selesai' ? 'selected' : '' }}>Selesai</option>
                                    <option value="dibatalkan" {{ old('status_reservasi_wisata', $reservasi->status_reservasi_wisata) == 'dibatalkan' ? 'selected' : '' }}>Dibatalkan</option>
                                </select>
...
{{-- <script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        let pesertaInput = document.getElementById("jumlah_peserta");
        let diskonInput = document.getElementById("nilai_diskon");
        let nilaiDiskonInput = document.getElementById("diskon");
        let totalBayarInput = document.getElementById("total_bayar");

        let paketHarga = @json($pakets->pluck('harga_per_pack', 'id'));

        function updateHarga() {
            let paketId = paketSelect.value;
            if (paketId && paketHarga[paketId]) {
                hargaInput.value = paketHarga[paketId];
                updateTotalBayar();
            } else {
                hargaInput.value = 0;
            }
        ...
        function updateTotalBayar() {
            ...
            let jumlahPeserta = parseInt(pesertaInput.value) || 1;
            let diskonPersen = parseFloat(diskonInput.value) || 0;

            let total = harga * jumlahPeserta;
            let nilaiDiskon = total * (diskonPersen / 100);
            let totalBayar = total - nilaiDiskon;

            nilaiDiskonInput.value = Math.round(nilaiDiskon);
            totalBayarInput.value = Math.round(totalBayar);
        }

        paketSelect.addEventListener("change", updateHarga);
        pesertaInput.addEventListener("input", updateTotalBayar);
        diskonInput.addEventListener("input", updateTotalBayar);

        // Initialize calculations on page load with existing values
        updateTotalBayar();
    });
</script> --}}
...
<script>
    ...
    function tampil_pesan(){
        ...
        if(pesan.innerHTML && pesan.innerHTML.trim() !== ''){
