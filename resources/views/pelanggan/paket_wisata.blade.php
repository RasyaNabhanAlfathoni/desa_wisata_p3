...
<div class="hero-wrap js-fullheight" style="background-image: url('{{ asset('front-end/images/lsp/wisata/jalanan utama/Penglipuran.jpg') }}');">
                        ...
                        <div class="form-group">
                            ...
                            @if(request()->has('paket_id') || request()->has('jumlah_peserta') ||
                               request()->has('date_from') || request()->has('harga_min'))
                                <a href="{{ url('pelanggan/paket-wisata') }}" class="btn btn-outline-secondary py-3 px-5 w-100 mt-2">
                                    Reset Pencarian
                                </a>
                            @endif
                        </div>
  ...
  <script>
    $(document).ready(function() {
        ...
        // Price range slider
        const hargaMin = {{ request('harga_min', 250000) }};
        const hargaMax = {{ request('harga_max', 500000) }};
        const maxHarga = {{ $paketWisatasAll->max('harga_per_pack') ?? 1000000 }};

        $("#harga-slider").slider({
            range: true,
            min: 0,
            max: maxHarga,
            values: [hargaMin, hargaMax],
            slide: function(event, ui) {
                $("input[name='harga_min']").val(ui.values[0]);
                $("input[name='harga_max']").val(ui.values[1]);
            }
        });

        // Set initial values
        $("input[name='harga_min']").val($("#harga-slider").slider("values", 0));
        $("input[name='harga_max']").val($("#harga-slider").slider("values", 1));

        // Update slider when input changes
        $("input[name='harga_min'], input[name='harga_max']").on('change', function() {
            $("#harga-slider").slider("values", [
                parseInt($("input[name='harga_min']").val()),
                parseInt($("input[name='harga_max']").val())
            ]);
        });
        ...
        $("form").on('submit', function() {
            ...
            if ($("#date_from").val() && $("#date_to").val() && dateFrom > dateTo) {
